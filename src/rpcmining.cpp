...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfWorkReward(999999,0);
}
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Audiocoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Audiocoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Audiocoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Audiocoin is downloading blocks...");
